...
async function reply(message, mode) {
  ...
  if (mode === 'ai') {
    if (!anthropicKey && !openaiKey && !geminiKey) {
      ...
      fallback.notice = "Mode IA indisponible (aucune cle ANTHROPIC_API_KEY, OPENAI_API_KEY ou GEMINI_API_KEY configuree sur le serveur) : reponse du moteur local a la place.";
      return fallback;
    }
    try {
