...
async function reply(message, mode) {
  ...
  if (mode === 'ai') {
    if (!anthropicKey && !openaiKey) {
      ...
      fallback.notice = "Mode IA indisponible (aucune cle ANTHROPIC_API_KEY ou OPENAI_API_KEY configuree sur le serveur) : reponse du moteur local a la place.";
      return fallback;
    }
    try {
      if (anthropicKey) return await anthropicReply(message, anthropicKey);
      return await openaiReply(message, openaiKey);
    } catch (err) {
      const fallback = localReply(message);
